...
class MediaDataFormater
{
    ...
    public JObject ImageSizeFormat(JArray Rja,JObject imgSz,double round,double scale)
    {
        ...
        {
            Console.WriteLine("ImageSizeFormat ERROR");
            //提示按任意键继续
    ...
    public void MediaDataFormat(string Jpath, string Fpath)
    {
        try
        {
            ...
            //询问是否使用1536位图尺寸
            Console.WriteLine("是否使用1536位图尺寸?如有中文1200位图尺寸需求请输入n或0并按回车键（不输入按回车默认执行1536位图尺寸）");
            //选项收录
            string sscale = Console.ReadLine();
            //判定是否直接回车，输入它值或直接回车则执行
            if (sscale == "0" || sscale == "n")
            {
                scale = 1;
            }
            else { }
